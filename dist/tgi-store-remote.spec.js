...
spec.test('lib/tgi-store-remote.spec.js', 'REMOTE_STORE', '', function (callback) {
  ...
  spec.heading('RemoteStore', function () {
    ...
    spec.heading('Store tests are applied', function () {
      spec.my_runnerStoreMethods(RemoteStore,true);
      spec.runnerListStoreIntegration(RemoteStore);
    });
  });
});

spec.my_runnerStoreMethods = function (SurrogateStore) {
  spec.heading('PROPERTIES', function () {
    spec.heading('name', function () {
      spec.example('name of store can be set in constructor', 'punchedCards', function () {
        return new SurrogateStore({name: 'punchedCards'}).name;
  ...
  spec.heading('METHODS', function () {
    ...
    spec.heading('getModel()', function () {
      if (services['canGetModel']) {
        ...
        if (services['isReady']) {
          spec.xexample('returns error when model not found', spec.asyncResults('Error: model not found in store'), function (callback) {
            var m = new Model();
            m.attributes[0].value = 1;
            new SurrogateStore().getModel(m, function (mod, err) {
              if (err) {
